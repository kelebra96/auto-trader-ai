...
class NotificationPreferencesSchema(Schema):
    expiry_alerts = fields.Boolean(missing=True)
    stock_alerts = fields.Boolean(missing=True)
    email_notifications = fields.Boolean(missing=True)
    push_notifications = fields.Boolean(missing=True)
    alert_days_before = fields.Integer(missing=7, validate=lambda x: 1 <= x <= 30)
...
@jwt_required()
@validate_json(NotificationPreferencesSchema())
@log_request
