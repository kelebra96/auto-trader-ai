...
const Profile = () => {
  ...
  const carregarPerfil = async () => {
    try {
      setLoading(true);
      const data = await userService.getPerfilAtual();
      setUserData(data);
      setFormData({
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await userService.updatePerfilAtual(formData);
  ...
  const handlePasswordSave = async () => {
    ...
    try {
      ...
      await userService.alterarSenhaAtual({
  ...
  const handlePhotoUpload = async (event) => {
    ...
    try {
      ...
      await userService.uploadFotoPerfil(formDataUpload);
