...
router.get('/', authenticate, requirePermission('alerts_view'), asyncHandler(async (req, res) => {
  ...
  const { count, rows: alertas } = await Alerta.findAndCountAll({
    ...
    include: [{
      model: Produto,
      as: 'produto',
      attributes: ['id', 'nome', 'codigo_barras', 'estoque_atual', 'estoque_minimo'],
      required: false
    }]
  });
...
// Criar novo alerta
router.post('/', authenticate, validate(schemas.createAlerta), asyncHandler(async (req, res) => {
  const alertaData = {
    ...req.body,
    usuario_id: req.user.id
  };
...
router.patch('/:id/marcar-lido', authenticate, validate(schemas.idParam), asyncHandler(async (req, res) => {
  const alerta = await Alerta.findOne({
    where: { 
      id: req.params.id,
      usuario_id: req.user.id
...
router.patch('/marcar-todos-lidos', authenticate, asyncHandler(async (req, res) => {
...
router.get('/nao-lidos/count', authenticate, asyncHandler(async (req, res) => {
...
router.post('/gerar/estoque-baixo', authenticate, asyncHandler(async (req, res) => {
...
router.post('/gerar/produtos-vencendo', authenticate, asyncHandler(async (req, res) => {
  ...
  for (const entrada of entradasVencendo) {
    ...
    if (!alertaExistente) {
      const diasRestantes = Math.ceil((entrada.data_validade - new Date()) / (1000 * 60 * 60 * 24));
      
      const alerta = await Alerta.create({
        tipo: 'produto_vencendo',
        titulo: `Produto vencendo: ${entrada.produto.nome}`,
        mensagem: `O produto "${entrada.produto.nome}" (Lote: ${entrada.lote}) vence em ${diasRestantes} dias (${entrada.data_validade.toLocaleDateString()})`,
        produto_id: entrada.produto_id,
        usuario_id: req.user.id
      });
