...
router.get('/', authenticate, asyncHandler(async (req, res) => {
  const { page = 1, limit = 10, empresa_id, produto_id, fornecedor_id, data_inicio, data_fim } = req.query;
...
router.post('/', authenticate, validate(schemas.createEntrada), asyncHandler(async (req, res) => {
...
// Atualizar entrada
router.put('/:id', authenticate, validate(schemas.idParam), validate(schemas.updateEntrada), asyncHandler(async (req, res) => {
  const entrada = await EntradaProduto.findOne({
    where: { id: req.params.id },
    include: [{
      model: Empresa,
  ...
  if (req.body.fornecedor_id && req.body.fornecedor_id !== entrada.fornecedor_id) {
    const fornecedor = await Fornecedor.findOne({
      where: { 
        id: req.body.fornecedor_id,
        empresa_id: entrada.empresa_id
...
router.get('/vencimento/proximos', authenticate, asyncHandler(async (req, res) => {
...
router.get('/vencimento/vencidos', authenticate, asyncHandler(async (req, res) => {
