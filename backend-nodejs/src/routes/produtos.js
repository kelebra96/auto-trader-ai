...
router.post(
  ...
  validate(schemas.createProduto),
  asyncHandler(async (req, res) => {
...
router.put(
  ...
  validate(schemas.idParam, "params"),
  validate(schemas.updateProduto),
  asyncHandler(async (req, res) => {
    const produto = await Produto.findOne({
      where: { id: req.params.id },
      include: [
    ...
    if (
      ...
      req.body.fornecedor_id !== produto.fornecedor_id
    ) {
      const fornecedor = await Fornecedor.findOne({
        where: {
          id: req.body.fornecedor_id,
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      !quantidade ||
      !operacao ||
      !["adicionar", "remover", "definir"].includes(operacao)
    ) {
      return res.status(400).json({
        error:
          "Quantidade e operação (adicionar, remover, definir) são obrigatórias",
      });
    }
