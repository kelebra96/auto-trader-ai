...
// Listar fornecedores
router.get('/', authenticate, requirePermission('suppliers_view'), asyncHandler(async (req, res) => {
...
router.post('/', authenticate, validate(schemas.createFornecedor), asyncHandler(async (req, res) => {
...
router.put('/:id', authenticate, validate(schemas.idParam), validate(schemas.updateFornecedor), asyncHandler(async (req, res) => {
  const fornecedor = await Fornecedor.findOne({
    where: { id: req.params.id },
    include: [{
      model: Empresa,
...
router.patch('/:id/toggle-status', authenticate, validate(schemas.idParam), asyncHandler(async (req, res) => {
  ...
  res.json({ message: `Fornecedor ${fornecedor.ativo ? 'ativado' : 'desativado'} com sucesso` });
}));
