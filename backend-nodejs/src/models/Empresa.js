    ...
    },
    {
      ...
      hooks: {
        beforeValidate: (empresa) => {
          ...
          // Normalizar CNPJ: aceitar tanto o formato mascarado quanto apenas dígitos
          if (empresa.cnpj) {
            const raw = String(empresa.cnpj).trim();
            const digits = raw.replace(/\D/g, "");
            if (digits.length === 14) {
              // Formatar para XX.XXX.XXX/XXXX-XX antes da validação e armazenamento
              empresa.cnpj = `${digits.slice(0, 2)}.${digits.slice(
                2,
                5
              ...
              // Mantém o valor como está (a validação 'is' irá rejeitar formatos inválidos)
              empresa.cnpj = raw;
            }
          }
  ...
  Empresa.addScope("withCounts", {
    attributes: {
      include: [
        [
          sequelize.literal(
            "(SELECT COUNT(*) FROM fornecedores WHERE fornecedores.empresa_id = Empresa.id)"
          ),
          "total_fornecedores",
        ],
        [
          sequelize.literal(
            "(SELECT COUNT(*) FROM produtos WHERE produtos.empresa_id = Empresa.id)"
          ),
          "total_produtos",
        ],
        [
          sequelize.literal(
            "(SELECT COUNT(*) FROM vendas WHERE vendas.empresa_id = Empresa.id)"
          ),
          "total_vendas",
        ],
      ],
